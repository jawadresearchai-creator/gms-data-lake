...
def build_v8_bridge(v7_bridge: Path, reclassification: Path, adjudication: Path, out_dir: Path) -> dict:
    ...
    try:
        con.execute(f"CREATE VIEW base AS SELECT * FROM read_parquet('{v7_bridge.as_posix()}')")
        ...
        ''')
        total = int(con.execute('SELECT COUNT(*) FROM v8').fetchone()[0])
        base_rows = int(con.execute('SELECT COUNT(*) FROM base').fetchone()[0])
        dup_company = int(con.execute('''SELECT COUNT(*) FROM (SELECT canonical_company_id FROM v8 GROUP BY canonical_company_id HAVING COUNT(*)>1)''').fetchone()[0])
        dup_inst = int(con.execute('''SELECT COUNT(*) FROM (SELECT openalex_institution_id FROM v8 GROUP BY openalex_institution_id HAVING COUNT(*)>1)''').fetchone()[0])
        null_keys = int(con.execute("SELECT COUNT(*) FROM v8 WHERE canonical_company_id IS NULL OR canonical_company_id='' OR openalex_institution_id IS NULL OR openalex_institution_id='' ").fetchone()[0])
    ...
    manifest = {
        ...
        'production_v8_rows': total,
        'promoted_company_ids': promote_ids,
        'kept_or_rejected_company_ids': non_promote_ids,
        'adjudication_policy': decisions.get('policy'),
    }
    (out_dir / 'organization_bridge_v8_manifest.json').write_text(json.dumps(manifest, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    return manifest


def main() -> int:
    p = argparse.ArgumentParser(description='Build v8 production organization bridge from manual adjudication')
    p.add_argument('--v7-bridge', type=Path, required=True)
    p.add_argument('--reclassification', type=Path, required=True)
    p.add_argument('--adjudication', type=Path, required=True)
    p.add_argument('--out-dir', type=Path, required=True)
    a = p.parse_args()
    print(json.dumps(build_v8_bridge(a.v7_bridge, a.reclassification, a.adjudication, a.out_dir), indent=2, sort_keys=True))
    return 0
