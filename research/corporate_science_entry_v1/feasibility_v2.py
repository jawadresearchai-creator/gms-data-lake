...
# DuckDB does not permit prepared parameters inside CREATE VIEW. These paths are
# fixed repository-local workflow inputs, so use SQL literals with explicit escaping.
def qp(path: str) -> str:
    return path.replace("'", "''")

con.execute(f"""
CREATE VIEW global_topic AS
SELECT CAST(publication_year AS INTEGER) year,
       CAST(topic_id AS VARCHAR) topic_id,
       CAST(work_count AS DOUBLE) work_count,
       CAST(citation_sum AS DOUBLE) citation_sum,
       CAST(mean_citations AS DOUBLE) mean_citations,
       CAST(topic_score_sum AS DOUBLE) topic_score_sum
FROM read_parquet('{qp(INPUTS['global_topic'])}')
WHERE publication_year BETWEEN 1900 AND 2026
""")
con.execute(f"""
CREATE VIEW company_topic AS
SELECT CAST(canonical_company_id AS VARCHAR) company_id,
       CAST(openalex_institution_id AS VARCHAR) openalex_institution_id,
       CAST(publication_year AS INTEGER) year,
       CAST(topic_id AS VARCHAR) topic_id,
       CAST(work_count AS DOUBLE) work_count,
       CAST(citation_sum AS DOUBLE) citation_sum,
       CAST(topic_score_sum AS DOUBLE) topic_score_sum
...
SELECT CAST(canonical_company_id AS VARCHAR) company_id,
       CAST(publication_year AS INTEGER) year,
       CAST(collaborator_country_code AS VARCHAR) collaborator_country_code,
       CAST(collaborative_work_count AS DOUBLE) collaborative_work_count,
       CAST(citation_sum AS DOUBLE) citation_sum
...
SELECT CAST(canonical_company_id AS VARCHAR) company_id,
       CAST(company_name AS VARCHAR) company_name,
       CAST(publication_year AS INTEGER) year,
       CAST(work_count AS DOUBLE) work_count,
       CAST(citation_sum AS DOUBLE) citation_sum,
       CAST(mean_citations AS DOUBLE) mean_citations
...
SELECT CAST(canonical_company_id AS VARCHAR) company_id,
       ...
       CAST(work_count AS DOUBLE) work_count,
       CAST(citation_sum AS DOUBLE) citation_sum,
       CAST(citations_per_work AS DOUBLE) citations_per_work
...
SELECT company_id, topic_id, MIN(year) entry_year
FROM company_topic WHERE work_count > 0
GROUP BY company_id, topic_id
""")

source_stats = {}
for table in ["global_topic", "company_topic", "company_collab", "company_research", "company_citation"]:
    n, lo, hi = con.execute(f"SELECT COUNT(*), MIN(year), MAX(year) FROM {table}").fetchone()
...
for scale in thresholds:
    ...
    WITH x AS (
      SELECT topic_id, year, work_count,
             LAG(work_count) OVER(PARTITION BY topic_id ORDER BY year) prev_wc,
             LEAD(work_count) OVER(PARTITION BY topic_id ORDER BY year) next_wc,
             AVG(work_count) OVER(PARTITION BY topic_id ORDER BY year ROWS BETWEEN 3 PRECEDING AND 1 PRECEDING) prior3_avg
      FROM global_topic
    ), crossing AS (
      SELECT * FROM x
    ...
    SELECT topic_id, MIN(year) emergence_year,
           ARG_MIN(work_count,year) work_count_at_emergence,
           ARG_MIN(prior3_avg,year) prior3_avg_at_emergence
    ...
    SELECT f.company_id, f.topic_id, f.entry_year, e.emergence_year,
           f.entry_year-e.emergence_year entry_lag,
           ...
           CASE WHEN f.entry_year-e.emergence_year BETWEEN -5 AND -1 THEN 'pioneer'
                ...
                ELSE 'too_early' END entry_group
    ...
    SELECT b.company_id,b.topic_id,c.collaborator_country_code,
           SUM(c.collaborative_work_count) collab_works
    FROM base b JOIN company_collab c
      ON c.company_id=b.company_id AND c.year BETWEEN b.entry_year-3 AND b.entry_year-1
    ...
    SELECT company_id,topic_id,COUNT(*) pre3_collab_countries,
           SUM(collab_works) pre3_collab_works,
           CASE WHEN SUM(collab_works)>0
                THEN 1.0-SUM(collab_works*collab_works)/(SUM(collab_works)*SUM(collab_works))
                ELSE NULL END pre3_network_diversity
    FROM net_country GROUP BY company_id,topic_id
    """)
    con.execute("""
    CREATE OR REPLACE TEMP TABLE pre AS
    SELECT b.company_id,b.topic_id,SUM(r.work_count) pre3_research_output,
           SUM(r.citation_sum) pre3_research_citations
    FROM base b JOIN company_research r
      ON r.company_id=b.company_id AND r.year BETWEEN b.entry_year-3 AND b.entry_year-1
    GROUP BY b.company_id,b.topic_id
    """)
    con.execute("""
    CREATE OR REPLACE TEMP TABLE post AS
    SELECT b.company_id,b.topic_id,SUM(t.work_count) post3_topic_output,
           SUM(t.citation_sum) post3_topic_citations,
           COUNT(DISTINCT t.year) post3_active_years
    FROM base b JOIN company_topic t
      ON t.company_id=b.company_id AND t.topic_id=b.topic_id
     AND t.year BETWEEN b.entry_year+1 AND b.entry_year+3
    GROUP BY b.company_id,b.topic_id
    """)
    con.execute(f"""
    ...
    SELECT b.*,n.pre3_collab_countries,n.pre3_collab_works,n.pre3_network_diversity,
           ...
           b.entry_year<=2021 has_full_3y_window
    FROM base b
    ...
    q = lambda sql: int(con.execute(sql).fetchone()[0])
    emergence_topics = q(f"SELECT COUNT(*) FROM {em} WHERE emergence_year BETWEEN 2000 AND 2021")
    episodes = q(f"SELECT COUNT(*) FROM {ep}")
    firms = q(f"SELECT COUNT(DISTINCT company_id) FROM {ep}")
    topics = q(f"SELECT COUNT(DISTINCT topic_id) FROM {ep}")
    pioneers = q(f"SELECT COUNT(*) FROM {ep} WHERE entry_group='pioneer'")
    early = q(f"SELECT COUNT(*) FROM {ep} WHERE entry_group='early'")
    middle = q(f"SELECT COUNT(*) FROM {ep} WHERE entry_group='middle'")
    late = q(f"SELECT COUNT(*) FROM {ep} WHERE entry_group='late'")
    full3 = q(f"SELECT COUNT(*) FROM {ep} WHERE has_full_3y_window")
    network_n = q(f"SELECT COUNT(*) FROM {ep} WHERE pre3_collab_countries IS NOT NULL")
    research_n = q(f"SELECT COUNT(*) FROM {ep} WHERE pre3_research_output IS NOT NULL")
    comparable_topics = q(f"""SELECT COUNT(*) FROM (
      SELECT topic_id FROM {ep} GROUP BY topic_id
      HAVING COUNT(DISTINCT company_id)>=2
         AND SUM(CASE WHEN entry_lag<=2 THEN 1 ELSE 0 END)>=1
         AND SUM(CASE WHEN entry_lag>=3 THEN 1 ELSE 0 END)>=1) x""")
    comparable_episodes = q(f"""SELECT COUNT(*) FROM {ep} x JOIN (
      SELECT topic_id FROM {ep} GROUP BY topic_id
      HAVING COUNT(DISTINCT company_id)>=2
         AND SUM(CASE WHEN entry_lag<=2 THEN 1 ELSE 0 END)>=1
         AND SUM(CASE WHEN entry_lag>=3 THEN 1 ELSE 0 END)>=1) c USING(topic_id)""")

    earlyish, later = pioneers+early, middle+late
    net_pct = 100*network_n/episodes if episodes else 0
    res_pct = 100*research_n/episodes if episodes else 0
    balance = min(earlyish,later)/max(earlyish,later) if max(earlyish,later) else 0
    passed = (comparable_topics>=50 and comparable_episodes>=150 and firms>=15
              and earlyish>=50 and later>=50 and full3>=100 and net_pct>=35)
    grid.append({
...
pool=passes or grid
preferred=sorted(pool,key=lambda r:(r["comparable_topics"],r["comparable_episodes"],r["early_late_balance"],r["scale_threshold"]),reverse=True)[0]
...
SELECT e.company_id,COALESCE(MAX(r.company_name),e.company_id) company_name,
       COUNT(*) episodes,COUNT(DISTINCT e.topic_id) topics,
       SUM(CASE WHEN e.entry_lag<=2 THEN 1 ELSE 0 END) earlyish,
       SUM(CASE WHEN e.entry_lag>=3 THEN 1 ELSE 0 END) later
FROM ep_s{s} e LEFT JOIN company_research r USING(company_id)
GROUP BY e.company_id ORDER BY episodes DESC
""").fetchall()
...
report={"project":"corporate_science_entry_v1","generated_at_utc":datetime.now(timezone.utc).isoformat(),
        ...
        "gate_criteria":{"comparable_topics_min":50,"comparable_episodes_min":150,
                         "firms_min":15,"earlyish_episodes_min":50,"later_episodes_min":50,
                         "full_3y_window_min":100,"pre3_network_coverage_pct_min":35.0}}
