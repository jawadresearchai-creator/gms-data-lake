...
import duckdb
from gmsdl.analytics.organization_bridge_v3 import build_v3_bridge
...
def test_build_v3_bridge(tmp_path: Path) -> None:
    v2=tmp_path/'v2.parquet'; r=tmp_path/'r.parquet'; adj=tmp_path/'adj.json'; out=tmp_path/'out'
    ddl='canonical_company_id VARCHAR,openalex_institution_id VARCHAR,reclassification_decision VARCHAR'
    _write(v2,ddl,[('C1','I1','KEEP_PRODUCTION')])
    _write(r,ddl,[('C2','I2','REVIEW_PRIORITY'),('C3','I3','REVIEW_PRIORITY')])
    ...
    m=build_v3_bridge(v2,r,adj,out)
    assert m['production_v3_rows']==2
